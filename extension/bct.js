...
Input should be comma separated Member IDs. (Maximum 30 members)`
	...
	modAPI.hookFunction("ChatRoomCharacterViewDrawOverlay", 2, (args, next) => {
		...
		// Let BC draw first so its icons cannot cover the BCT overlay.
		const result = next(args);
		const showAllIcons = (typeof ChatRoomHideIconStateType !== "undefined")
			? ChatRoomHideIconState === ChatRoomHideIconStateType.SHOW_ALL
			: ChatRoomHideIconState === 0;

		if (C.BCT && showAllIcons
		&& (!Player.BCT.bctSettings.bctIconOnlyShowOnHover
		|| MouseHovering(CharX, CharY, 500 * Zoom, 70 * Zoom))) {
	...
	modAPI.patchFunction("ElementButton._ParseIcons",{
		"src = `./Assets/Female3DCG/ItemMisc/Preview/${icon}.png`;" : parseIconReplaceBy,
	})

	modAPI.hookFunction("DialogLockingClick",2, (args,next) => {
		let ClickedLock = args[0];
		let C = args[1];
		...
		if((args[0].Asset.Name === BF_LOCK_NAME || args[0].Asset.Name === BF_TIMER_LOCK_NAME) && DialogMenuMode !== "permissions" && !InventoryBlockedOrLimited(C, ClickedLock)) {
			...
			args[0].Asset = AssetGet(C.AssetFamily,"ItemMisc","HighSecurityPadlock");
		}
		next(args);
		if(!!CurrentItem) {
			if(LockType === BF_LOCK_NAME) convertHStoBF(C,CurrentItem,CharFocusGroup);
			else if(LockType === BF_TIMER_LOCK_NAME) convertHStoBFTimer(C,CurrentItem,CharFocusGroup);
		}
	})
